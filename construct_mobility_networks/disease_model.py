...
class Model:
    ...
    def init_exogenous_variables(self,
                                 ...
                                 track_full_history_for_all_CBGs=True, # was False
        ...
        ########## VACCINATION ##########
        if cbg_idx_to_vaccinate is not None:
            self.VACCINATE = True
        else:
            self.VACCINATE = False
        
        if self.VACCINATE:
            self.VAXED_CBGS = cbg_idx_to_vaccinate
            print("vaxed cbgs: ")
        ...
        if self.VACCINATE:
            ...
            D = sparse.diags(multiplier, dtype=poi_cbg_visits_list[0].dtype)
            self.POI_CBG_VISITS_LIST = [matrix * D for matrix in poi_cbg_visits_list]
        # print(self.POI_CBG_VISITS_LIST[1][:, :10])
        #################################
        ...
        self.P_SICK_AT_T0 = p_sick_at_t0  # percentage of CBG that starts off sick # set here
    ...
    def _build_proportions_matrix(self, proportion_dicts):
        ...
        assert (self.POI_CBG_PROPORTIONS.sum(axis=1) <= 1 + 1e-6).all()
            ...
            #print((self.POI_CBG_PROPORTIONS.sum(axis=1) < .5).mean())
    ...
    def init_endogenous_variables(self, simulation_type='prior'):

        #self.expectation = np.zeros(self.num_seeds)
        #self.variance = np.zeros(self.num_seeds)
        #print(type(self.CBG_SIZES))
        #print(self.CBG_SIZES * self.P_SICK_AT_T0)
        first_exp = np.sum(self.CBG_SIZES * self.P_SICK_AT_T0)
        self.expectation = np.full((self.num_seeds), first_exp)
        ...
        # only do this for prior
        # if self.VACCINATE and simulation_type == 'prior':
        #     multiplier = np.ones(self.N)
        #     multiplier[self.VAXED_CBGS] = 0.
        #     self.P0 = self.P0 * multiplier

            # P0 definitely begins the same for all sims
        ...
        self.history = {}
        print('different groups being tracked:')
        for group in self.cbg_idx_groups_to_track:
            group_idxs = self.cbg_idx_groups_to_track[group]
            if group == 'nyt':
                print("Number of NYT groups to track: " + str(len(group_idxs)))
            self.history[group] = {}
    ...
    def simulate_disease_spread(self, simulation_type='prior',
                                verbosity=24,
        ...
        '''
        self.start_eval_hour = start_eval_hour
        ...
        while t < self.T:
            ...
            if t == self.start_eval_hour: # interrupt simulation at eval vax hour
                if self.simulation_type == 'prior':
                    break
        ...
        print(f'Average number of people infected across random seeds: {np.mean(total):.3f}') # final reported number is E+I+R
        print('Number of CBGs tracked for simulation:' + str(self.N))

        ##### SIMULATION OUTPUTS #####
        self.all_infected = np.mean(all_infected, axis=0) # this is size 1 seed x n. cbgs, its the NEIR of each cbg, the NUMBER not fraction

        self.cbg_latent_out = np.mean(np.sum(self.cbg_latent, axis=1))
        self.cbg_infected_out = np.mean(np.sum(self.cbg_infected, axis=1))
        self.cbg_removed_out = np.mean(np.sum(self.cbg_removed, axis=1))
        self.total_for_each_seed = np.sum(all_infected, axis=1)

        self.all_infected_std = np.std(all_infected, axis=0) # std for above
        self.total_infected = np.mean(total) # I added this # this is right
        self.total_infected_std = np.std(total) # this is right
        ...
        self.total_protected = np.mean(np.sum(num_sus, axis=1)) # summed over rows to get total protected per seed, then take mean over the seeds
        self.Pi = np.mean(sus_frac, axis=0) # fraction protected for each CBG, averaged over seeds

        # EIR fractions between 0 and 1 
        self.latent_fraction = np.divide(self.cbg_latent,self.CBG_SIZES)
        self.infected_fraction = np.divide(self.cbg_infected, self.CBG_SIZES)
        self.removed_fraction = np.divide(self.cbg_removed, self.CBG_SIZES)
        all_infected_fraction = self.latent_fraction + self.infected_fraction + self.removed_fraction
        self.all_infected_fraction = np.mean(all_infected_fraction, axis=0)
        self.all_infected_fraction_std = np.std(all_infected_fraction, axis=0)
    ...
    def update_history(self, t):
        if self.track_full_history_for_all_CBGs:
            ...
            self.full_history_for_all_CBGs['latent'].append(self.cbg_latent.mean(axis=0) / self.CBG_SIZES) # mean across seeds
            self.full_history_for_all_CBGs['infected'].append(self.cbg_infected.mean(axis=0) / self.CBG_SIZES) # mean across seeds
            self.full_history_for_all_CBGs['removed'].append(self.cbg_removed.mean(axis=0) / self.CBG_SIZES) # mean across seeds
    ...
    def get_new_cases(self, t):
        ...
        if self.VACCINATE: # set susceptible counts to zero for vaxed cbgs
            if (self.simulation_type == 'eval' and t > self.start_eval_hour): # or self.simulation_type == 'prior':
                multiplier = np.ones(self.N)
                multiplier[self.VAXED_CBGS] = 0.
                num_sus = num_sus * multiplier

        sus_frac = num_sus / self.CBG_SIZES  # S x N, number susceptible in each cbg
        ...
        if self.PSI > 0:
            ...
            if self.use_home_proportion_beta:
                ...
                cbg_base_infection_rates = self.HOME_BETA * cbg_prop_home * cbg_densities  # S x N, infected proportion at home cbg
            else:
        ...
        if self.POI_CBG_VISITS_LIST is not None:  # try to load
            #print("not re-computing visits list")
            poi_cbg_visits = self.POI_CBG_VISITS_LIST[t]  # M x N, get visits at time t 
            poi_visits = poi_cbg_visits @ np.ones(poi_cbg_visits.shape[1])  # M, faster than summing axis=1, visits at each poi 
        ...
        #         seed_poi_cbg_infecte[d = seed_poi_cbg_sus.transpose().multiply(seed_poi_infection_rates).transpose()  # M x N
    ...
    def get_new_infectious(self):
        ...
        '''
        new_infectious = np.random.binomial(self.cbg_latent.astype(int), 1 / self.LATENCY_PERIOD) # HERE
    ...
    def get_new_removed(self):
        ...
        '''
        new_removed = np.random.binomial(self.cbg_infected.astype(int), 1 / self.INFECTIOUS_PERIOD) # HERE
        return new_removed
